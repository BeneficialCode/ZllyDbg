...
void SymbolHandler::EnumSymbols(std::string mask) {
	SymEnumSymbols(_hProcess, _baseAddress, mask.c_str(), [](PSYMBOL_INFO pSymbol, ULONG size, PVOID context) {
		printf("0x%p %s\n", pSymbol->Address, pSymbol->Name);
		return TRUE;
		}, this);
}

void SymbolHandler::EnumTypes(std::string mask) {
	SymEnumTypesByName(_hProcess, _baseAddress, mask.c_str(), [](PSYMBOL_INFO pSymbol, ULONG size, PVOID context) {
		printf("0x%p %s\n", pSymbol->Address, pSymbol->Name);
		return TRUE;
		}, this);
}
...
ULONG64 SymbolHandler::GetSymbolAddressFromName(std::string Name) {
	...
	SymFromName(_hProcess, Name.c_str(), pSymbol);
...
DWORD SymbolHandler::GetStructMemberOffset(std::string Name, std::string memberName) {
	...
	for (ULONG i = 0; i < childrenParams->Count; i++) {
		...
		printf("member offset: 0x%x name: %s\n", offset, pMemberInfo->Name);
	}
...
bool SymbolHandler::GetSymbolFromName(std::string name, PSYMBOL_INFO pSymbol) {
	return ::SymFromName(_hProcess, name.c_str(), pSymbol);
}

bool SymbolHandler::GetSymbolFromIndex(DWORD64 dllBase, int index, PSYMBOL_INFO pSymbol) {
	return ::SymFromIndex(_hProcess, dllBase, index, pSymbol);
}
